...
(function (root, factory) {
  ...
  function updateSummary(summary) {
    document.querySelector('[data-total-icons]')?.replaceChildren(String(summary.total));
  ...
  function createMap(maplibregl, data) {
    ...
    map.once('load', async () => {
      try {
        ...
        if (status) status.textContent = `${data.summary.total} фигурок · ${data.summary.rivers.representedSystems} речные системы`;
      } catch (error) {
        console.error(error);
        const status = document.getElementById('map-status');
        if (status) {
  ...
  function diagnostics(map, data) {
    ...
    return {
      ...
      pixelRatio: map.getPixelRatio?.() || Math.min(window.devicePixelRatio || 1, 1.5),
