(function (root, factory) {
  ...
  function specialRadius(hitArea = false) {
    ...
    return [
      'interpolate', ['linear'], ['zoom'],
      config.minZoom, hitArea ? roleRadius(14, 16, 18) : roleRadius(4.2, 5.0, 5.8),
      11, hitArea ? roleRadius(17, 19, 21) : roleRadius(5.0, 5.8, 6.8),
      15, hitArea ? roleRadius(20, 22, 24) : roleRadius(6.0, 7.0, 8.2)
    ];
  }
  ...
  function createStyle(data) {
    return {
      ...
      layers: [
        ...
        {
          id: config.specialPointLayerId, type: 'circle', source: config.specialSourceId,
          paint: {
            'circle-radius': specialRadius(false),
            'circle-color': config.colors.gold,
            'circle-stroke-color': config.colors.goldOutline,
            'circle-stroke-width': ['interpolate', ['linear'], ['zoom'], config.minZoom, 1.15, 15, 1.8],
            'circle-opacity': 1,
            'circle-stroke-opacity': 0.98
          }
        },
        {
          id: config.specialHitLayerId, type: 'circle', source: config.specialSourceId,
          paint: {
            'circle-radius': specialRadius(true),
            'circle-color': '#000000',
  ...
  function buildSpriteMetrics(manifest) {
    ...
    for (const icon of manifest.icons) metrics.set(icon.id, {...icon, center_x: icon.width / 2, center_y: icon.height / 2});
    return metrics;
  }
  ...
  function createMountainImageLayer(maplibregl, data, atlasImage) {
    ...
    const features = [...data.icons.features].sort((left, right) => {
      ...
      return (b.latitude - a.latitude) || (a.priority - b.priority) || a.point_id.localeCompare(b.point_id);
    });
    ...
    return {
      ...
      onAdd(map, gl) {
        ...
        for (const feature of features) {
          ...
          const baseWidth = BASE_WIDTH_M[properties.source_type];
          if (!baseWidth) throw new Error(`Неизвестный исходный размер для ${properties.point_id}: ${properties.source_type}`);
          const widthM = baseWidth * Number(properties.icon_scale) * SIZE_MULTIPLIER;
  ...
  async function loadAtlasImage(manifest) {
    ...
    if (manifest.atlas_format === 'base64-png') {
      const encoded = (await loadChunkedText(manifest.atlas)).trim();
  ...
  function showFeatureCard(properties) {
    ...
    card.querySelector('[data-feature-role]')?.replaceChildren(roleLabel);
    card.querySelector('[data-feature-name]')?.replaceChildren(String(properties.name || ''));
    card.querySelector('[data-feature-elevation]')?.replaceChildren(`${Math.round(Number(properties.elevation_m))} м`);
    const note = card.querySelector('[data-feature-note]');
    if (note) {
      const text = String(properties.semantic_note || '').trim();
      note.replaceChildren(text);
      note.toggleAttribute('hidden', !text);
    }
    card.dataset.featureId = String(properties.id || '');
    card.removeAttribute('hidden');
  }

  function bindSpecialMountainInteractions(map) {
    const canvas = map.getCanvas();
    map.on('mouseenter', config.specialHitLayerId, () => { canvas.style.cursor = 'pointer'; });
    map.on('mouseleave', config.specialHitLayerId, () => { canvas.style.cursor = ''; });
    map.on('click', config.specialHitLayerId, (event) => {
      const feature = event.features?.[0];
      if (feature?.properties) showFeatureCard(feature.properties);
    });
    document.querySelector('[data-action="close-feature"]')?.addEventListener('click', () => {
      document.getElementById('feature-card')?.setAttribute('hidden', '');
    });
  }
  ...
  function createMap(maplibregl, data) {
    ...
    const map = new maplibregl.Map({
      container: 'map', style: createStyle(data), center, zoom: config.minZoom, minZoom: config.minZoom, maxZoom: config.maxZoom,
      maxBounds: expandedBounds(data.bounds), pitch: 0, maxPitch: 0, bearing: 0, dragRotate: false, pitchWithRotate: false, touchPitch: false,
      renderWorldCopies: false, attributionControl: false, antialias: false, fadeDuration: 0, preserveDrawingBuffer: false,
    ...
    map.once('load', async () => {
      try {
        ...
        bindSpecialMountainInteractions(map);
        ...
        if (status) status.textContent = `${data.summary.pngTotal} фигурок · ${data.summary.specialTotal} особых точек · ${data.summary.rivers.representedSystems} речные системы`;
      } catch (error) {
        console.error(error);
        const status = document.getElementById('map-status');
  ...
  function diagnostics(map, data) {
    ...
    return {
      ...
      pixelRatio: map.getPixelRatio?.() || Math.min(window.devicePixelRatio || 1, 1.5), sourceIds, layerIds, runtimeLayerOrder,
      pointCount: data.summary.total, pngPointCount: data.summary.pngTotal, specialPointCount: data.summary.specialTotal,
      counts: data.summary.counts, specialCounts: data.summary.specialCounts,
      ...
      imageAnchorMode: renderer?.anchorMode || null, imageSizeMultiplier: renderer?.sizeMultiplier || 0,
      specialSourceRegistered: Boolean(style.sources?.[config.specialSourceId]), pointInteractionEnabled: layerIds.includes(config.specialHitLayerId),
      ...
      riverLineBeforeSpecialPoints: runtimeLayerOrder.indexOf(config.riverLineLayerId) < runtimeLayerOrder.indexOf(config.specialPointLayerId),
      specialPointBeforeHitArea: runtimeLayerOrder.indexOf(config.specialPointLayerId) < runtimeLayerOrder.indexOf(config.specialHitLayerId)
    };
  }
