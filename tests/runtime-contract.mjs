...
assert.match(uiSource, /const VERSION = '7\.3\.1'/);
...
assert.match(page, /allowFullFileFallback:true/);
...
assert.match(indexSource, /map-presentation-r2\.js\?v=7\.3\.1-snow4/);
...
assert.equal(data.version, '7.3.1');
assert.equal(data.applicationVersion, '7.3.1');
...
if (data.regionalSnow?.available) {
  ...
  assert.equal(data.dataVersion, '7.3.1-snow-refined-z12-area-match.3');
  ...
  assert.equal(data.regionalSnow.displayStrategy, 'stable-vector-detailed-area-match');
  assert.equal(data.regionalSnow.displayMaxzoom, 12);
  assert.equal(data.regionalSnow.displaySourceZoom, 12);
